"""
XSER training loop using transition reward selection.

This loop stores episodes based on high transition-level reward and periodically
repeats actions from selected valuable trajectories.
"""
    ...
    for total_step_counter in range(int(max_steps_training)):
        ...
        if total_step_counter < max_steps_exploration or explore:
            ...
            print(
                ...
                f"crucial_episode_num:{crucial_episode_num}"
            )
        ...
        if total_step_counter > batch_size:
            if (not memory.long_term_memory.is_full()) or (
                memory.long_term_memory.is_full()
                and total_reward > memory.long_term_memory.get_min_reward()
